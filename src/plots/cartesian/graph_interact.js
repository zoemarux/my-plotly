...
fx.init = function(gd) {
    ...
    subplots.forEach(function(subplot) {
        ...
        // trello123: x軸およびy軸ドラッグによるスクロール機能は不要（チャート自体のズーム機能は生かすためplotly.js側を修正）
    ...
    // trello71: HoverLayer event handlers below originally get TypeError. Skip TypeError for tooltip's close <a> tag.
    ...
    hoverLayer.onmousemove = function(evt) {
        try{
            ...
            if(e.name.toUpperCase() !== TYPE_ERROR){
            　   throw e;
            }
        }
...
function hover(gd, evt, subplot) {
            ...
            if(newPoints) {
                ...
                for(var newPointNum = 0; newPointNum < newPoints.length; newPointNum++) {
                    ...
                    if(isNumeric(newPoint.x0) && isNumeric(newPoint.y0)) {
                        if (newPoint.text && rwUtils.wpIsIos()) {
                            // trello83: appended <a> tag for changing tooltip clickable for some reason.
                            newPoint.text = newPoint.text + '<a></a>';
                        }
    ...
    var tooltipCnPn = hoverData[0].trace.index + '_' + hoverData[0].index;
    var skipHover = false;

    Plotly.d3.selectAll('g.annotation').each(function(da, ia){
      if (Plotly.d3.select(this).attr("cn_pn") == tooltipCnPn) {
        skipHover = true
      }
    })
...
function createHoverText(hoverData, opts) {
    ...
    hoverLabels.each(function(d) {
        ...
        if(name && name !== text) {
            tx2.style('fill', COLOR_RGB_BLACK)
                .text(name)
                .call(Drawing.setPosition, 0, 0)
                .attr('data-notex', 1)
                .call(svgTextUtils.convertToTspans);
        ...
        g.select('path')
            ...
            .style('fill-opacity', BG_FILL_OPACITY);
    ...
    // trello46: tooltip toggling for iOS
    // Details:
    // - Using plotly_beforehover event instead of plotly_hover.
    //   (because plotly_hover event is not triggered when touching the same plot continuously.)
    if (rwUtils.wpIsIos()) {
        // Create class name string with an unique key that uses tooltip transform(translate) attr info.
        var targetFixedHoverTextClass = 'hovertext_fixed_' + d3.select(hoverLabels[0][0]).attr('transform').replace( /(\.|\,|\(|\))/g , "_" );
        var exisistingTooltips = container.select('g.' + targetFixedHoverTextClass);

        if (exisistingTooltips[0][0]) {
            // Target has already been existing.
            //   -> Remove the exisiting tooltip.
            //   -> Also remove 'hovertext' class tooltip.
            container.selectAll('g.' + targetFixedHoverTextClass).remove();
            container.selectAll('g.hovertext').remove();
        } else {
            // Target has NOT been existing.
            //   -> New tooltip with 'hovertext' class has already been created by prior codes.
            //   -> To fix the tooltip, replace created tooltip's 'hovertext' class with targetFixedHoverTextClass.
            var addClassObj = {};
            addClassObj[targetFixedHoverTextClass] = true;

            // trello83: bind handler to tooltip itself for easily closing tooltip.
            // (used mousemove ev instead of click ev because click ev needs one additinal touch compared with mousemove ev.)
            d3.select(hoverLabels[0][0]).classed('hovertext', false).classed(addClassObj).on("mousemove", function(d) {
                this.remove();
            });
        }


        // trello62: for iOS: Change plot color from gray to red when clicking plot in custom mode and no legend
        var traces = document.getElementsByClassName('traces');
        var customBtn = document.querySelector('#mode-switch > button.custom');
        // In custom mode and no legend?
        if (customBtn.className.indexOf('active') >= 0 && traces.length === 0) {
            var points = document.querySelectorAll('g.point path');

            if (points.length > 0) {
                var pointIndex = hoverData[0].index;
                var pointFillColor = points[pointIndex].style['fill'];
                var COLOR_RGB_GRAY = 'rgb(211, 211, 211)';
                var COLOR_HEX_GRAY = '#d3d3d3';
                var COLOR_HEX_RED = '#d93321';
                var COLOR_HEX_WHITE = '#fff';

                // *** CAUTION *** for iOS device: COLOR_RGB_GRAY, for iOS simulator: COLOR_HEX_GRAY
                if (pointFillColor == COLOR_RGB_GRAY || pointFillColor == COLOR_HEX_GRAY) {
                    // Change plot color to red
                    points[pointIndex].style['fill'] = COLOR_HEX_RED;
                    // Change tooltip color to red
                    var tooltipPath = container.select("g." + targetFixedHoverTextClass + " path");
                    var tooltipText = container.select("g." + targetFixedHoverTextClass + " text");
                    if (tooltipPath[0][0] && tooltipText[0][0]) {
                        tooltipPath[0][0].style['fill'] = COLOR_HEX_RED;
                        tooltipPath[0][0].style['stroke'] = COLOR_HEX_WHITE;
                        tooltipText[0][0].style['fill'] = COLOR_HEX_WHITE;
                    }
                } else {
                    // Change plot color to gray
                    points[pointIndex].style['fill'] = COLOR_HEX_GRAY;
                }
            }
        }
