...
exports.makeEditable = function(context, _delegate, options) {
    ...
    function appendEditable() {
        var plotDiv = d3.select(Lib.getPlotDiv(that.node())),
            ...
            textWidth = context[0][0].getBBox().width,
            textHeight = context[0][0].getBBox().height,
            annoBgColor = 'transparent';

        if (context[0][0].previousElementSibling && context[0][0].previousElementSibling.style["fill"]) {
            annoBgColor = context[0][0].previousElementSibling.style["fill"]
        }

        div.classed('plugin-editable editable', true)
            .style({
                'min-height': (textHeight + 10) + 'px',
                'min-width': (textWidth + 10) + 'px',
                position: 'absolute',
                'font-family': that.style('font-family') || 'Arial',
                'font-size': that.style('font-size') || 12,
                color: options.fill || that.style('fill') || 'black',
                opacity: 1,
                'background-color': annoBgColor,
            ...
            .html(options.text || that.attr('data-unformatted'))
            ...
            .on('blur', function() {
                ...
                if (text.length == 0 || text == "<br>") {
                  ...
                  newText = text.replace('<div>', '<br>').replace(/<div>/g, '').replace(/<\/div>/g, '<br>');
                }
                that.text(newText)
                    .style({opacity: 1});
                ...
                dispatch.edit.call(that, newText);
            })
            .on('focus', function() {
                var context = this;
            ...
            .on('keydown', function() {
                if(d3.event.which === 13 && !d3.event.shiftKey) {
