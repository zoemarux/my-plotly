...
module.exports = function draw(gd) {
    ...
    if(opts.height <= legendHeight || gd._context.staticPlot) {
        ...
        if (legendOrientation == "h") {
          legendRectWidth = (legendWidth * traces[0].length) + 200; // trello422: 術式名を全文表示
        }
        bg.attr({
...
function setupTraceToggle(g, gd) {
    ...
    traceToggle.on('click', function() {
        ...
        if(Registry.traceIs(trace, 'pie')) {
            ...
            // trello230: レジェンドクリック時のトレース強調処理をより強くする(WhytPlot_ChartTest:src/render_chart.jsに処理を移管のためコメントアウト)
            // if(legendgroup === '') {
